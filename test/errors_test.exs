defmodule ErrorsTest do
  ...
  test "closed connection triggered by Neo4j when a bad key type is used: LIST" do
    conn = Bolt.Sips.conn
    assert_raise Bolt.Sips.Exception, "Port #{inspect conn} is closed", fn ->
      Bolt.Sips.query!(conn, "CREATE (report:Report) SET report = {props}", %{props: %{'foo' => "bar"}})
    end
  end
end