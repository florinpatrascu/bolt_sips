defmodule Bolt.Sips.Internals.PackStream.Message.DecoderTest do
  ...
  test "Decode common message" do
    Enum.each(BoltVersionHelper.available_versions(), fn bolt_version ->
      ...
      assert {:success, %{"server" => "Neo4j/3.4.1"}} == Decoder.decode(success_hex, bolt_version)
      ...
      assert failure == Decoder.decode(failure_hex, bolt_version)

      # RECORD
      assert {:record, [1]} == Decoder.decode(<<0xB1, 0x71, 0x91, 0x1>>, bolt_version)

      # IGNORED
      assert {:ignored, _} = Decoder.decode(<<0xB0, 0x7E>>, bolt_version)
    end)
  end
end
