defmodule Bolt.Sips.Internals.PackStream.Message.EncoderV1Test do
  ...
  test "Encode ACK_FAILURE" do
    assert <<0x0, 0x2, 0xB0, 0xE, 0x0, 0x0>> == EncoderV1.encode({:ack_failure, []}, 1)
  end

  test "Encode DISCARD_ALL" do
    assert <<0x0, 0x2, 0xB0, 0x2F, 0x0, 0x0>> == EncoderV1.encode({:discard_all, []}, 1)
  end

  describe "Encode INIT:" do
    test "without params" do
      assert <<0x0, _, 0xB2, 0x1, _::binary>> = EncoderV1.encode({:init, []}, 1)
    end

    test "with params" do
      assert <<0x0, _, 0xB2, 0x1, _::binary>> = EncoderV1.encode({:init, [{"neo4j", "test"}]}, 1)
    end
  end

  test "Encode PULL_ALL" do
    assert <<0x0, 0x2, 0xB0, 0x3F, 0x0, 0x0>> == EncoderV1.encode({:pull_all, []}, 1)
  end

  test "Encode RESET" do
    assert <<0x0, 0x2, 0xB0, 0xF, 0x0, 0x0>> == EncoderV1.encode({:reset, []}, 1)
  end
  ...
  describe "Encode RUN:" do
    test "without params" do
      assert <<0x0, 0x13, 0xB2, 0x10, 0x8F, 0x52, 0x45, 0x54, 0x55, 0x52, 0x4E, 0x20, 0x35, 0x20,
               ...
               0x0>> == EncoderV1.encode({:run, ["RETURN 5 AS num"]}, 1)
    end
