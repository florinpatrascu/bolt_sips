defmodule Bolt.Sips.Internals.PackStream.Message.EncoderV3Test do
  ...
  describe "Encode BEGIN" do
    test "without params" do
      assert <<0x0, 0x3, 0xB1, 0x11, 0xA0, 0x0, 0x0>> == :erlang.iolist_to_binary(EncoderV3.encode({:begin, []}, 3))
    end
  ...
  test "Encode COMMIT" do
    assert <<0x0, 0x2, 0xB0, 0x12, 0x0, 0x0>> == :erlang.iolist_to_binary(EncoderV3.encode({:commit, []}, 3))
  end

  test "Encode GOODBYE" do
    assert assert <<0x0, 0x2, 0xB0, 0x02, 0x0, 0x0>> == :erlang.iolist_to_binary(EncoderV3.encode({:goodbye, []}, 3))
  end

  describe "Encode HELLO" do
    test "without params" do
      assert <<0x0, _, 0xB1, 0x1, _::binary>> = :erlang.iolist_to_binary(EncoderV3.encode({:hello, []}, 3))
    end

    test "with params" do
      assert <<0x0, _, 0xB1, 0x1, _::binary>> = :erlang.iolist_to_binary(EncoderV3.encode({:hello, [{"neo4j", "test"}]}, 3))
    end
  end

  test "Encode ROLLBACK" do
    assert <<0x0, 0x2, 0xB0, 0x13, 0x0, 0x0>> == :erlang.iolist_to_binary(EncoderV3.encode({:rollback, []}, 3))
  end
  ...
  describe "Encode RUN" do
    ...
    test "without params but with metadata" do
      ...
      assert <<0x0, 0x22, 0xB3, 0x10, 0x8F, 0x52, 0x45, 0x54, 0x55, 0x52, 0x4E, 0x20, 0x31, 0x20,
               ...
               0x0>> ==
               :erlang.iolist_to_binary(EncoderV3.encode({:run, ["RETURN 1 AS num", %{}, metadata]}, 3))
    end
    ...
    test "with params but without metadata" do
      assert <<0x0, 0x1E, 0xB3, 0x10, 0xD0, 0x13, 0x52, 0x45, 0x54, 0x55, 0x52, 0x4E, 0x20, 0x7B,
               ...
               0x0>> ==
               :erlang.iolist_to_binary(EncoderV3.encode({:run, ["RETURN {num} AS num", %{num: 5}]}, 3))
    end
    ...
    test "with params and  metadata" do
      ...
      assert <<0x0, 0x2C, 0xB3, 0x10, 0xD0, 0x13, 0x52, 0x45, 0x54, 0x55, 0x52, 0x4E, 0x20, 0x7B,
               ...
               0x0>> ==
               :erlang.iolist_to_binary(EncoderV3.encode({:run, ["RETURN {num} AS num", %{num: 5}, metadata]}, 3))
    end
  end
end
