defmodule Bolt.Sips.Internals.PackStream.Message.EncoderTest do
  ...
  test "Encode common messages" do
    Enum.each(BoltVersionHelper.available_versions(), fn bolt_version ->
      # ACK FAILTURE
      assert <<0x0, 0x2, 0xB0, 0xE, 0x0, 0x0>> == Encoder.encode({:ack_failure, []}, bolt_version)

      # DISCARD_ALL
      assert <<0x0, 0x2, 0xB0, 0x2F, 0x0, 0x0>> ==
               Encoder.encode({:discard_all, []}, bolt_version)

      # INIT
      assert <<0x0, _, 0xB2, 0x1, _::binary>> = Encoder.encode({:init, []}, bolt_version)

      assert <<0x0, _, 0xB2, 0x1, _::binary>> =
               Encoder.encode({:init, [{"neo4j", "test"}]}, bolt_version)

      # PULL_ALL
      assert <<0x0, 0x2, 0xB0, 0x3F, 0x0, 0x0>> == Encoder.encode({:pull_all, []}, bolt_version)

      # RESET
      assert <<0x0, 0x2, 0xB0, 0xF, 0x0, 0x0>> == Encoder.encode({:reset, []}, bolt_version)

      # RUN
      assert <<0x0, 0x13, 0xB2, 0x10, 0x8F, 0x52, 0x45, 0x54, 0x55, 0x52, 0x4E, 0x20, 0x31, 0x20,
               0x41, 0x53, 0x20, 0x6E, 0x75, 0x6D, 0xA0, 0x0,
               0x0>> == Encoder.encode({:run, ["RETURN 1 AS num"]}, bolt_version)
      ...
      assert <<0x0, 0x1D, 0xB2, 0x10, 0xD0, 0x13, 0x52, 0x45, 0x54, 0x55, 0x52, 0x4E, 0x20, 0x7B,
               ...
               0x75, 0x6D, 0x5, 0x0,
               0x0>> == Encoder.encode({:run, ["RETURN {num} AS num", %{num: 5}]}, bolt_version)

      # Bug fix: encoding struct fails
      ...
      assert <<0x0, 0x39, 0xB2, 0x10, 0xD0, 0x17, 0x43, 0x52, 0x45, 0x41, 0x54, 0x45, 0x20, 0x28,
               ...
               _::binary>> = Encoder.encode({:run, [query, params]}, bolt_version)
    end)
  end
end
